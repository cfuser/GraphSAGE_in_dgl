...
user_features = g.nodes['user'].data['features']
item_features = g.nodes['item'].data['features']
# print('----')
# print(user_features.size())
node_features = {'user': user_features.to(device = torch.device('cuda')), 'item': item_features.to(device = torch.device('cuda'))}
...
for i in range(1, epoch + 1):
    res = model(g, {'user': user_features.to('cuda:0'), 'item': item_features.to('cuda:0')})
